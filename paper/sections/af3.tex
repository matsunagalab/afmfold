...
Specifically, while \AFiii employs Denoising Diffusion Probabilistic Models (DDPMs) following the variance-preserving (VP) formulation, we compute the likelihood of the generated structure with respect to the specified CVs and add it to the original \AFiii score as a driving force that increases this likelihood. With this modified score, the generated structures become explicitly conditioned on the CVs.
...
Sampling the generative process of \AFiii can be written as the stochastic EDM sampler \cite{karras2022elucidating}:
\begin{equation}
    \begin{split}
    ...
    \label{eq:af3-sde}
\end{equation}
Here, $\hat{t}_i$ denotes a time slightly later than $t_i$, defined as $\hat{t}_i = (1 + \gamma_i)t_i$, with a parameter $\gamma_i$ that is set to $0.8$ when $t_{i+1} > 1.0$, and $0$ otherwise. 
