\subsection{Guiding \AFiii with inter-domain distance restraints}
...
Sampling in \AFiii can be written as the reverse-time variance-preserving SDE (VP-SDE):
...
where $\beta_t$ is the noise schedule, $\bar{W}_t$ is a standard Wiener process, and the score $\nabla_{X_t}\log \pt{X_t}$ is approximated by a denoising network $s_\theta(X_t,t,a)$.
We integrate \eqref{af3-sde} backward from $t=1$ to $t=0$.

Let $\phi_{\text{object}} \in \mathbb{R}^D$ denote the target vector of inter-domain distances, where $D$ is the number of domain pairs, and let $\phi: X \to \mathbb{R}^D$ be a differentiable mapping from structures to this feature space.
...
Here, $\eta_t$ is a (possibly time-dependent) guidance strength.
