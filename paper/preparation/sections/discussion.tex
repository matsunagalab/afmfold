...
In this work, we have proposed a computational protocol, \Model, to estimate the 3D conformation of biomolecules from AFM images. 
...
\Model circumvents molecular dynamics simulations entirely, which fundamentally changes the computational cost structure compared with existing flexible fitting approaches.
In conventional flexible fitting \cite{Niina2020, ngo2025atomic}, each AFM frame requires an independent MD simulation lasting hours to days, making the analysis of large image sequences prohibitive.
Normal-mode-based approaches such as NMFF-AFM \cite{Wu2024} and AFMFit \cite{Vuillemot2025AFMfit} achieve even faster per-frame processing (tens of seconds per frame on standard hardware) through iterative application of normal-mode linear response, and can capture large-amplitude anharmonic motions \cite{amyot_flexible_2025}.
However, these methods rely on normal-mode bases derived from a given force field, so the accessible conformational space is shaped by the choice of force field and the number of modes retained.
In contrast, \Model leverages the co-evolutionary information embedded in \AFiii to generate conformations without depending on MD force fields or normal-mode bases, offering a complementary approach.
...
These factors set a practical lower bound on the conformational differences that \Model can reliably detect from experimental images and should be kept in mind when interpreting the modest numerical margins observed in the c.c.\ comparisons---where the metric is inherently insensitive to conformational detail---and the larger, structurally grounded separation in the embedding-distance comparisons (\Cref{fig:resnet_embeddings}~(b,c)).
...
Extending the framework to estimate uncertainty would make the estimates easier to interpret \cite{Kendall2017Uncertainty}.
