...
In rigid-body fitting, a candidate 3D structure is placed in a trial orientation, a simulated image is rendered from it, and the pixel-wise similarity (e.g., cross-correlation) between this rendered image and the observed AFM image is computed; the pose and structure are then iteratively updated to maximize this similarity.
...
We use this g-CNN to estimate CVs from each single AFM image, which then guide the structure generation in \AFiii (as described in the following subsection).
