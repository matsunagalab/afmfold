...
Structurally, it comprises four domains (\acdi, \acdii, \acdiii, and \acdiv) and a linker to the transmembrane domain (FlhA\textsubscript{TM}) (\Cref{fig:resnet_embeddings}~(d)). 
...
Using the embedding-based fitting described above, we projected the 256-dimensional embedding vectors onto two dimensions (\Cref{fig:resnet_embeddings}~(a)).
...
In this structural feature space, the \Model estimates (blue dots in \Cref{fig:resnet_embeddings}~(a, left)) cluster more closely around the experimental images (red stars) than the crystal structure does (green dots in \Cref{fig:resnet_embeddings}~(a, right)).
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        \textbf{(a)}~Two-dimensional projection of the embedding vectors.
        ...
        \textbf{(d)}~Structure of \flhac with domain coloring.
        \textbf{(e--g)}~Individual cases: (left) \Model structure in c.c.-minimized pose,
        (center) experimental AFM image, (right) \Model structure fitted by c.c.\ maximization.
    }
