...
We then trained the g-CNN on this ensemble and evaluated its prediction accuracy using pseudo-AFM images generated from randomly selected MD conformations.
...
We applied \Model to all 159 experimental AFM images. For each frame, we rendered simulated AFM images from the \Model prediction (and, separately, from the crystal structure) and performed two types of rigid-body fitting over the imaging parameters (3D rotation, translation, and tip radius): one maximizing the c.c.\ and the other minimizing the geodesic distance in the embedding space (see \Cref{subsubsec:evaluation}).
...
In this structural feature space, the \Model predictions (blue dots in \Cref{fig:resnet_embeddings}~(d, left)) cluster more closely around the experimental images (red stars) than the crystal structure does (green dots in \Cref{fig:resnet_embeddings}~(d, right)).
This is particularly noteworthy because \Model does not use the ResNet-18 model at all---it relies solely on the \gcnn for CV estimation---yet its predictions are independently validated by the ResNet-18 embedding space.
Quantitatively, the mean geodesic distance between experimental images and \Model predictions was $0.101 \pm 0.044$, compared with $0.125 \pm 0.046$ for the crystal structure (\Cref{fig:resnet_embeddings}~(b)).
...
\begin{figure}[htbp]
    ...
    \caption{
        ...
        Left: \Model predictions (blue) and experimental images (red stars).
