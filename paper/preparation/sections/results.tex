...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{images/figure-3.png}
    \caption{
...
\Model allows estimation of conformations from each AFM image in approximately one minute on a single GPU (considering training time, total computation time is less than the conventional flexible fitting methods). 
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{images/figure-5.png}
    \caption{
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{images/figure-6.png}
    \caption{
        ...
        \textbf{(i)} Scatter plot comparing the L2 distances from the experimental image to the \Model estimation ($y$-axis) and to the crystal structure ($x$-axis). Points below the diagonal indicate cases where \Model achieves a closer match.
        \textbf{(j)} Bland--Altman plot of the c.c.\ values. The $x$-axis shows the mean of $\mathrm{c.c._{AFMFold}}$ and $\mathrm{c.c._{Crystal}}$, and the $y$-axis shows their difference. Points are colored by the \acdii--\acdiv inter-domain distance. The dashed line indicates the mean difference.
    }
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{images/figure-7.png}
    \caption{
...
First, HS-AFM captures molecular dynamics on a timescale of 1~ms per frame, which may reflect longer-timescale motions than those sampled by AlphaFlow or BioEmu, thereby enabling the observation of more open structures.
...
The auto-correlations from \Model estimations (solid lines) show a pronounced positive correlation for lag times up to 2.0~ms and oscillatory behavior at longer lag times.
