...
1. **Prepare a candidate conformation set**  
    In the paper, diverse structures were generated by running AlphaFold 3 with various restraints (see [`../notebooks/example.ipynb`](../notebooks/example.ipynb) to check how to add restraints on AlphaFold 3).
    Candidate conformation set can also be extracted from long MD simulations.

2. **Compute CVs corresponding to the trajectory**  
    For example, to compute inter-domain distances from a MD trajectory, you can use:
